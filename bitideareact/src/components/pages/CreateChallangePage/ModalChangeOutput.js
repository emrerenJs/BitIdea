...
const ModalChangeOutput = (props) => {
    ...
    const prepareAndChangeOutput = () => {
        ...
        if(globalDimension === 0){
            ...
            for(let z = 0; z < dimension[0]; z++){
                ...
                for(let y = 0; y < dimension[1]; y++){
                    value[z].push([]);
                    for(let x = 0; x < dimension[2]; x++){
                        value[z][y].push(document.getElementById(`scenario ${props.scenarioIndex} input 0 ${y} ${x}`).value);
                    }
                }
            }
        }
        ...
        props.changeOutput(props.scenarioIndex,output);
        toggle();
    }
    ...
    return (
        <li>
            <div className={createChallangeStyle.addButton} onClick={toggle}>
                <div
                    className={createChallangeStyle.addButtonInside} style={{fontWeight:'bold', backgroundColor:'brown'}}>
                    Ou
                </div>
            </div>
            <Modal isOpen={modal} toggle={toggle} className={className}>
                ...
                <ModalFooter>
                    <Button color="primary" onClick={prepareAndChangeOutput}>Değiştir</Button>{' '}
                </ModalFooter>
            </Modal>
        </li>
    );
...
export default ModalChangeOutput;